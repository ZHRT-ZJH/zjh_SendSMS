...
class SmsSendConn
{
    ...
    //单条发送、相同内容群发、个性化内容群发、查询余额、请求获取上行、请求获取状态报告
    public $port_action = ['send_single', 'send_batch', 'send_multi', 'get_balance', 'get_mo', 'get_rpt'];
    ...
    public function  __construct($BaseUrl = null)
    {
        if($this->debug === true){
            //南方短信节点url地址
            $BaseUrl = 'http://api01.monyun.cn:7901/sms/v2/std/';

            //北方短信节点url地址
//            $this->BaseUrl = 'http://api02.monyun.cn:7901/sms/v2/std/';

//            $this->port_action = ['single_send', 'batch_send', 'multi_send', 'get_balance', 'get_mo', 'get_rpt'];
        }
    ...
    */
    public function singleSend($data)
    {
        try {
            ...
            $result = $this->connection($this->BaseUrl . $this->port_action[0], $post_data);//根据请求类型进行请求
   ...
   */
    public function batchSend($data)
    {
        try{
            ...
            $data['content']=$this->encrypt_content($data['content']);//短信内容进行urlencode加密
            $post_data=json_encode($data);//将数组转化为JSON格式
            $result = $this->connection($this->BaseUrl . $this->port_action[1], $post_data);//根据请求类型进行请求
            return $result;
        }catch (Exception $e) {
            print_r($e->getMessage());  //输出捕获的异常消息
        }
    ...
    */
    public function multiSend($data)
    {
        try{
            ...
            $post_data=json_encode($data);//将数组转化为JSON格式
            $result=$this->connection($this->BaseUrl . $this->port_action[2], $post_data);//根据请求类型进行请求
            return $result;
        }catch (Exception $e) {
            print_r($e->getMessage());  //输出捕获的异常消息
        }
   ...
   */
    public function getBalance($data)
    {
        try{
			...
			$data['timestamp']=$encrypt['time'];//获取加密时间戳
            $post_data=json_encode($data);//将数组转化为JSON格式
            $result=$this->connection($this->BaseUrl . $this->port_action[3], $post_data);//根据请求类型进行请求
            return $result;
        }catch (Exception $e) {
            print_r($e->getMessage());  //输出捕获的异常消息
        }
    ...
    */
    public function getMo($data)
    {
        try{
			...
			$data['timestamp']=$encrypt['time'];//获取加密时间戳
            $post_data = json_encode($data);//将数组转化为JSON格式
            $result=$this->connection($this->BaseUrl . $this->port_action[4], $post_data);//根据请求类型进行请求
            return $result;//返回请求结果
        }catch (Exception $e) {
            print_r($e->getMessage());  //输出捕获的异常消息
        }
 ...
 */
    public function getRpt($data)
    {
        try{
			...
			$data['timestamp']=$encrypt['time'];//获取加密时间戳
            $post_data = json_encode($data);//将数组转化为JSON格式
            $result=$this->connection($this->BaseUrl . $this->port_action[5], $post_data);//根据请求类型进行请求
            return $result;
        }catch (Exception $e) {
            print_r($e->getMessage());  //输出捕获的异常消息
        }
