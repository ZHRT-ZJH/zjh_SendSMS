...
try {
    ...
    if ($result['result'] === 0) {
        ...
        print_r("单条信息发送失败，错误码：" . $result['result'] . ($result['result_msg']? ("，错误信息：". $result["result_msg"]) : ''));
    }
}catch (Exception $e) {
...
try {
    ...
    if ($result['result'] === 0) {
        ...
        print_r("相同内容信息发送失败，错误码：" . $result['result'] . ($result['result_msg']? ("，错误信息：". $result["result_msg"]) : ''));
    }
}catch (Exception $e) {
    print_r($e->getMessage());//输出捕获的异常消息，请根据实际情况，添加异常处理代码
    return false;
...
try {
    ...
    if ($result['result'] === 0) {
        ...
        print_r("个性化内容信息发送失败，错误码：" . $result['result'] . ($result['result_msg']? ("，错误信息：". $result["result_msg"]) : ''));
    }
}catch (Exception $e) {
    print_r($e->getMessage());//输出捕获的异常消息，请根据实际情况，添加异常处理代码
    return false;
...
try {
    ...
    if ($result['result'] === 0) {
        ...
        print_r("查询余额失败，错误码：" . $result['result'] . ($result['result_msg']? ("，错误信息：". $result["result_msg"]) : ''));
    }
}catch (Exception $e) {
    print_r($e->getMessage());//输出捕获的异常消息，请根据实际情况，添加异常处理代码
    return false;
...
try {
    ...
    {
        print_r("获取上行失败，错误码：" . $result['result'] . ($result['result_msg']? ("，错误信息：". $result["result_msg"]) : ''));
    }
}catch (Exception $e) {
    print_r($e->getMessage());//输出捕获的异常消息，请根据实际情况，添加异常处理代码
    return false;
...
try {
    ...
    {
        print_r("获取状态报告失败，错误码：" . $result['result'] . ($result['result_msg']? ("，错误信息：". $result["result_msg"]) : ''));
    }
}catch (Exception $e) {
    print_r($e->getMessage());//输出捕获的异常消息，请根据实际情况，添加异常处理代码
    return false;
